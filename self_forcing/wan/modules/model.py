...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, crossattn_cache=None, semantic_decision=None):
        r"""
        Args:
        ...
        if semantic_decision is not None:
            # x = flash_attention(q, k * (1 + semantic_decision[:, :, None, None]), v, k_lens=context_lens)
            x = flash_attention(
                q,
                k * semantic_decision[:, :, None, None],
                v * semantic_decision[:, :, None, None],
                k_lens=context_lens
            )
        else:
