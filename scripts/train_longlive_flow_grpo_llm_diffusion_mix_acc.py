...
class TextPromptVideoDataset(Dataset):
    def __init__(self, dataset, split="train", debug=False):
        ...
        if split == "train" and dataset != "temporal_order":
            ...
            if debug:
                self.new_prompts = self.new_prompts[:8]
...
def eval_fn(
    ...
    for test_batch in tqdm(
        ...
        with torch.no_grad():
            collate_data = pipeline.flowgrpo_llm_diffusion_mix_inference(
                    noise=sampled_noise,
                    text_prompts_list=prompts,
                    low_memory=False,
                    inner_diffusion_group=1,
                    inner_diffusion_frame_step=12,
        ...
        for prompt_metadata_item in prompt_metadata:
            prompt_metadata_item["train_frames"] = 12
            prompt_metadata_item["inner_group"] = 1
            prompt_metadata_item["gap_frame"] = 12
...
def main(_):
    ...
    if accelerator.is_main_process:
        ...
        if config.debug:
            ...
            wandb.init(project="longlive_llm_qwen3vl", name=config.run_name, config=config.to_dict(), dir=log_dir)
    ...
    # ---- 加载 longlive 模型 ----
    pipeline = InteractiveCausalInferencePipeline(config.pretrained.self_config, device=device)
    longlive_config = config.pretrained.self_config

    if longlive_config.generator_ckpt:
        state_dict = torch.load(longlive_config.generator_ckpt, map_location="cpu")
        raw_gen_state_dict = state_dict["generator_ema" if longlive_config.use_ema else "generator"]

        if longlive_config.use_ema:
    ...
    if getattr(longlive_config, "adapter", None) and configure_lora_for_model is not None:
        pipeline.generator.model = configure_lora_for_model(
            pipeline.generator.model,
            model_name="generator",
            lora_config=longlive_config.adapter,
            is_main_process=True,
        )

        # Load LoRA weights (if lora_ckpt is provided)
        lora_ckpt_path = getattr(longlive_config, "lora_ckpt", None)
        if lora_ckpt_path:
            # if local_rank == 0:
            ...
            pipeline.generator.model = (
                pipeline.generator.model.merge_and_unload()
            )
    ...
    ema = None
    if config.train.ema:
        ema = EMAModuleWrapper(transformer_trainable_parameters, decay=config.train.ema_decay, update_step_interval=1, device=device)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        # gathered_rewards_dict["train_latents"] = gather_tensor_to_all(collated_samples["train_latents"], accelerator)
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                if local_attn_cfg != -1:
                    ...
                    # global attention
                    kv_cache_size = (diffusion_sample_frame + 3) * pipeline.frame_seq_length
                        ...
                        if current_start_frame != 0:
                            with torch.no_grad():
                                with accelerator.autocast():
                                    kv_cache1, crossattn_cache = pipeline._recache_after_switch_input_cache(
                                        train_sample_batch["latents_clean"], kv_cache1, crossattn_cache, current_start_frame, prompt_embeds_for
                                    )
