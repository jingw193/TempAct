...
import re
import os
from flow_grpo.qwen3vl_video_reward import qwen3vl_video_score
...
def main(csv_path, end_index=100, save_path=None, sup_test=False, reward_score_fn=None):
    if reward_score_fn is None:
        reward_score_fn = qwen3vl_video_score("cuda")
    if "simple100" in csv_path:
        metadata = [{"eval_sample_frames": 6, "gap_frame": 12, "num_votes": 1, "start_frame": 2}]
    else:
        metadata = [{"eval_sample_frames": 10, "gap_frame": 12, "num_votes": 1, "start_frame": 2}]

    df = pd.read_csv(csv_path)
    if save_path is None:
        csv_dir = os.path.dirname(csv_path)
        save_path = os.path.join(csv_dir, "qwen3vl_score_results.json")
    ...
    for index, row in df.iterrows():
        ...
        if data is None:
            ...
            try:
                ...
                print(f"Error computing qwen3vl score for index {index}: {e}")
                fail += 1
                continue
...
if __name__ == "__main__":
    ...
    for exp_name in os.listdir(root_path):
        ...
        csv_path = os.path.join(exp_path, "0521_Qwen3-VL-8B-Instruct_results.csv")
        if os.path.exists(csv_path):
            main(csv_path, end_index=100, sup_test=True, reward_score_fn=reward_score_fn)
