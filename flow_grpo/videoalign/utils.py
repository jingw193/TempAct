...
def load_model_from_checkpoint(
    ...
    if os.path.exists(full_ckpt):
        ...
        # Create a new state_dict to store modified key-value pairs
        new_state_dict = {}

        # 修复键名不匹配：移除多余的 .model 层级
        # checkpoint 有 base_model.model.model.*，但模型期望 base_model.model.*
        for key, value in model_state_dict.items():
            if key.startswith("base_model.model.model."):
                # 移除一个 .model: base_model.model.model.* -> base_model.model.*
