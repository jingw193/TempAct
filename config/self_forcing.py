...
def _get_self_config_text_grpo_diffusion_flowgrpo_mix(base_model="sd3", n_gpus=1, gradient_step_per_epoch=1, dataset="pickscore", reward_fn={}, name=""):
    config = base.get_config()
    assert base_model in ["sd3", "self_forcing"]
    assert dataset in ["pickscore", "ocr", "geneval", "temporal_order"]

    config.base_model = base_model
    config.dataset = dataset

    self_config = OmegaConf.load("/path/to/TempAct/self_forcing/config/self_forcing_dmd.yaml")
    default_config = OmegaConf.load("/path/to/TempAct/self_forcing/config/default_config.yaml")
    config.pretrained.self_config = OmegaConf.merge(default_config, self_config)
    ...
    # llm config
    config.train.llm_learning_rate = 2e-5
...
def self_forcing_llm_diffusion_rl_mix():
    reward_fn = {
    ...
    config = _get_self_config_text_grpo_diffusion_flowgrpo_mix(
        base_model="self_forcing", n_gpus=32, gradient_step_per_epoch=2, dataset="temporal_order", reward_fn=reward_fn, name="self_forcing_llm_diffusion_rl_mix"
    )
    return config
